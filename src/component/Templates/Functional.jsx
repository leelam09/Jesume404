...
export default function Functional({ resumeData }) {
  const {
    personalInfo,
    experience,
    education,
  ...
  return (
    <div className="bg-stone-50 w-full h-full font-sans print:bg-white">
      {/* Header - With image on left side */}
      <header className="bg-stone-900 text-white py-8 relative print:bg-stone-800 print:py-6">
        <div className="absolute top-0 left-0 w-full h-1 bg-amber-400 print:h-1.5"></div>
        <div className="max-w-7xl mx-auto px-8 flex flex-wrap items-center">
          {/* Profile Image - Now on the left */}
          <div className="mr-8 mb-4 md:mb-0">
            {personalInfo?.profileImage ? (
              <img
                src={personalInfo.profileImage}
                alt={personalInfo.name}
                className="w-28 h-28 rounded-full object-cover border-4 border-white shadow-md bg-white print:border-2"
              />
            ) : (
              <div className="w-28 h-28 rounded-full bg-stone-800 border-4 border-white shadow-md flex items-center justify-center text-amber-400 text-4xl print:border-2">
                <FaUser />
              </div>
            )}
          </div>
          ...
          {/* Name and Title - Now on the right of image */}
          <div className="flex-grow">
            <h1 className="text-4xl font-light tracking-wider mb-1 print:text-3xl">
              {personalInfo?.name?.toUpperCase() || "YOUR NAME"}
            </h1>
            <p className="text-amber-400 text-xl tracking-wide print:text-lg">
              {personalInfo?.title || "Professional Title"}
            </p>
          </div>
        </div>
      ...
      {/* Contact Info Bar */}
      <div className="bg-stone-100 py-4 px-8 flex flex-wrap justify-center border-b border-stone-200 print:bg-white print:border-stone-300">
        <div className="flex flex-wrap gap-6 justify-center md:justify-start max-w-7xl mx-auto">
          {personalInfo?.email && (
            <a
              href={`mailto:${personalInfo.email}`}
      ...
      <div className="max-w-7xl mx-auto p-8 print:p-6">
        ...
        {personalInfo?.summary && (
          <section className="mb-10 print:mb-8">
            <div className="flex items-center mb-4">
              <div className="w-10 h-10 bg-stone-200 rounded-full flex items-center justify-center mr-3 print:bg-stone-100">
                <FaUser className="text-amber-600" />
              </div>
              <h2 className="text-2xl text-stone-800 font-light tracking-wide print:text-xl">
                PROFESSIONAL SUMMARY
              </h2>
            </div>
            <div className="pl-14">
              <p className="text-stone-700 leading-relaxed border-l-2 border-amber-400 pl-5 py-1 italic print:text-stone-800">
                {personalInfo.summary}
              </p>
            </div>
          </section>
        ...
        {/* Two column layout */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 print:gap-8">
          {/* Left Column - Skills, Education, Certificates */}
          <div className="md:col-span-1">
            {/* Skills Section */}
            {skills?.length > 0 && (
                ...
                <div className="pl-14">
                  <div className="flex flex-wrap gap-2">
                    {skills.map((skill, index) => (
                      <span
                        key={index}
                        className="px-3 py-2 bg-stone-200 text-stone-800 rounded-md text-sm hover:bg-amber-100 transition-colors print:bg-stone-100"
                      >
                        {skill}
                      </span>
                    ))}
            ...
            {education?.length > 0 && (
              <section className="mb-10 print:mb-8">
                ...
                <div className="pl-14 space-y-5 print:space-y-4">
                  {education.map((edu, index) => (
                    <div key={index} className="border-l-2 border-stone-300 pl-4 py-1">
                      <h3 className="text-lg font-medium text-stone-800 print:text-base">
                        {edu.degree}
                      </h3>
                      <p className="text-amber-700 font-medium print:text-amber-800">{edu.school}</p>
                      <p className="text-stone-600 text-sm">
                        {edu.startDate} — {edu.endDate || "Present"}
                      </p>
                      {edu.location && (
                ...
                <div className="pl-14 space-y-5 print:space-y-4">
                  {certificates.map((cert, index) => (
                    <div key={index} className="border-l-2 border-stone-300 pl-4 py-1">
                      ...
                      <p className="text-amber-700 font-medium print:text-amber-800">
                        {cert.issuer}
                        {cert.credentialID && (
                          <span className="text-stone-500 text-sm ml-2">
                            ID: {cert.credentialID}
                      ...
                      {cert.url && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center text-amber-700 hover:text-amber-800 text-sm mt-1 print:text-amber-800"
                        >
                          <FaCertificate className="mr-1" />
                          <span>View Certificate</span>
                        </a>
          ...
          <div className="md:col-span-2">
            ...
            {experience?.length > 0 && (
              <section className="mb-10 print:mb-8">
                ...
                <div className="pl-14 space-y-8 print:space-y-6">
                  {experience.map((job, index) => (
                    <div key={index} className="relative">
                      <div className="flex flex-col md:flex-row md:justify-between mb-2 border-b border-stone-200 pb-2">
                        <h3 className="text-xl font-semibold text-stone-800 print:text-lg">
                          {job.position || job.title}
                        </h3>
                        <p className="text-amber-600 font-medium print:text-amber-700">
                          {job.startDate} — {job.endDate || "Present"}
                        </p>
                      </div>
                      <p className="text-amber-800 font-medium mb-2">
                      ...
                      <p className="text-stone-700 print:text-stone-800">{job.description}</p>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {achievements?.length > 0 && (
              <section className="mb-10 print:mb-8">
                ...
                <div className="pl-14 space-y-6 print:space-y-4">
                  {achievements.map((achievement, index) => (
                    <div key={index} className="relative">
                      <div className="flex flex-col md:flex-row md:justify-between mb-1">
                        <h3 className="text-lg font-semibold text-stone-800 print:text-base">
                          {achievement.title}
                        </h3>
                        {achievement.date && (
                          <p className="text-amber-600 text-sm print:text-amber-700">
                            {achievement.date}
                          </p>
                        )}
                      </div>
                      ...
                      {achievement.description && (
                        <p className="text-stone-700 print:text-stone-800">
                          {achievement.description}
                        </p>
                      )}
                    </div>
              ...
              <section className="mb-10 print:mb-8">
                ...
                <div className="pl-14 space-y-6 print:space-y-4">
                  {projects.map((project, index) => (
                    <div
                      key={index}
                      className="bg-white p-6 rounded-lg border-l-4 border-amber-400 shadow-sm hover:shadow-md transition-shadow print:border-l-2 print:shadow-none print:p-4"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="text-xl font-semibold text-stone-800 print:text-lg">
                          {project.title}
                        ...
                        {project.link && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-amber-600 hover:text-amber-800 print:text-amber-700"
                          >
                            View Project
                          </a>
                        )}
                      ...
                      <p className="text-stone-700 print:text-stone-800">{project.description}</p>
                    </div>
                  ))}
                </div>
              </section>
      ...
      <style jsx global>{`
        @media print {
          ...
          /* Improve text contrast for PDF */
          p, li {
            line-height: 1.5;
          }
          
          /* Enhance spacing for printable resume */
          section {
